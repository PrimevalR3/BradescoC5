package com.r3.da.app.restapi.service.token.withdraw
...
@Component
class DAWithdrawService(
    @Autowired
    val cordaApi: CordaApi,

    @Value("\${corda.legal-identity.issuer}")
    private val issuerLegalIdentity: String,

    @Value("\${com.r3.org.digitalasset.withdwarPlacement.fullyqualifiedclassname}")
    private val withdrawPlacementFlowClassname: String,
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(DAWithdrawService::class.java)
    }

    private fun getIdentityShortHashForLegalIdentity(): String {
    ...
    fun withdrawPlacement(assetRef: String): FlowStatusResponse {
        ...
        cordaApi.startFlow(holdingIdentityShortHash,
            FlowStartRequest(flowStartRequestId,
                withdrawPlacementFlowClassname,
                " {\n" + " \"assetRef\" : \"${assetRef}\"\n" +"}")
        )

...
@InitiatingFlow(protocol = "finalize-update-collateral-loan-flow")
class UpdateCollateralFlow(): AbstractFlow(), ClientStartableFlow {
    @Suspendable
    override fun call(requestBody: ClientRequestBody): String {
        logger.info("${this::class.java.enclosingClass}.call() called")
        try {
            val myInfo = memberLookup.myInfo()
            val flowArgs = requestBody.getRequestBodyAs(json, UpdateCollateral::class.java)
            val collateralState = ledgerService.findUnconsumedStatesByType(CollateralTokenState::class.java).filter { loan ->
                loan.state.contractState.collateralId.toString() == flowArgs.collateralId
            }
            ...
            logger.warn("create collateral failed")
            throw e
        }
    }
}

@InitiatedBy(protocol = "finalize-update-collateral-flow")
class FinalizeUpdateCollateralTokenFlow: AbstractFlow(), ResponderFlow {
